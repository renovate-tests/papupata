...
export default function invokeSupertest<Arg,Ret> (supertestRequest: ReturnType<typeof supertest>, api: MockableAPI<Arg, Ret>, args: Arg) {
   ...
   // for some reason ts refuses to accept that the value can be changed from null without the as any
    ...
    api.apiDeclaration.configure({
      ...config, makeRequest:
      async (method:string, url:string, query:any, body:any) => {
